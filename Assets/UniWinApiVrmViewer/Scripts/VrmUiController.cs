...
public class VrmUiController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            // プロパティをバインド
            if (transparentToggle) { transparentToggle.onValueChanged.AddListener((value => windowController.isTransparent = value)); }
            if (maximizeToggle) { maximizeToggle.onValueChanged.AddListener((value => windowController.isZoomed = value)); }
            if (topmostToggle) { topmostToggle.onValueChanged.AddListener((value => windowController.isTopmost = value)); }
        }
    ...
    public void UpdateUI()
    {
        if (!windowController)
        {
            ...
            if (maximizeToggle) { maximizeToggle.isOn = windowController.isZoomed; }
